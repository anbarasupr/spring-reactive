...
public class FluxAndMonoGeneratorService {
    ...
    public Flux<String> namesFlux_map(int stringLength) {
        ...
        return Flux.fromIterable(namesList)
                ...
                .map(s -> s.length() + "-" + s)
                .doOnNext(name -> {
                    System.out.println("onNext name is : " + name);
    ...
    public Flux<String> namesFlux_transform(int stringLength) {
        ...
        var namesList = List.of("alex", "ben", "chloe"); // a, l, e , x
        return Flux.fromIterable(namesList)
                //.map(String::toUpperCase).filter(s -> s.length() > stringLength)
    ...
    public Flux<String> explore_merge() {
        ...
        var abcFlux = Flux.just("A", "B", "C")
                .delayElements(Duration.ofMillis(100)); // every 100th sec, emits one by one:  A B C

        var defFlux = Flux.just("D", "E", "F")			//every 125th sec, emits one by one:   D E F
